...
export default function GateModal() {
  ...
  const handleLogin = () => {
    if (isClerkConfigured) {
      ...
      alert("Clerk is not configured. Set NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY in .env.local");
    }
  };

  // Clerk SignIn overlay
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="relative w-full max-w-md bg-[#18181b] rounded-2xl p-6 shadow-2xl">
        <button
          onClick={closeGate}
        ...
        <div className="flex flex-col gap-3">
          {isUpgrade ? (
            <a
              href="/pricing"
              className="w-full py-2.5 rounded-lg bg-gradient-to-r from-green-500 to-green-600 text-white font-medium text-center hover:from-green-600 hover:to-green-700 transition-all"
            >
              Upgrade to Pro
            </a>
          ) : (
          ...
          <button
            onClick={closeGate}
            className="w-full py-2.5 rounded-lg text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 transition-colors text-sm"
          >
            Maybe Later
          </button>
        </div>
...
// Dynamically-imported SignIn component
