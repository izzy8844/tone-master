...
export default function MidiSetup() {
  ...
  const setMidiPort = usePlaybackStore((s) => s.setMidiPort);
  ...
  const refreshPorts = useCallback(async () => {
    ...
    if (currentMidiPort) {
      const found = portList.find((p) => p.id === currentMidiPort);
      if (found) {
        selectPort(currentMidiPort);
        setStep("ready");
        return;
      }
    }
  ...
  return (
    <div className="rounded-lg border border-zinc-700 p-4 space-y-3">
      ...
      <div className="space-y-1.5">
        {ports.map((p) => {
          const isSelected = currentMidiPort === p.id;
          return (
            <button
              key={p.id}
      ...
      {step === "ready" && (
        <div className="space-y-2">
          ...
          {testResult && (
            <p className="text-xs text-zinc-500">{testResult}</p>
          )}
        </div>
      )}
    </div>
...
function MacIACGuide() {
  return (
    <div className="border border-blue-500/30 bg-blue-500/5 rounded-lg p-3 space-y-2">
      ...
      <a
        href="https://support.apple.com/guide/audio-midi-setup/transfer-midi-information-between-apps-ams1013/mac"
        target="_blank"
  ...
  return (
    <div className="border border-purple-500/30 bg-purple-500/5 rounded-lg p-3 space-y-2">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="text-xs text-purple-400 hover:underline inline-block"
      >
